...
public class MultiPart: NSObject {
    ...
    public static let mime = "mime"
    public static let fileextension = "extension"
    public static let data = "data"

    var session: URLSession?
    public func callPostWebService(_ url_String: String, parameetrs: [String: Any]?, filePathArr arrFilePath: [[String: Any]]?, completion: @escaping ([String: Any]?, Error?) -> Void) {
        ...
        request.setValue("Bearer " + UserDefaultsManager.accessToken, forHTTPHeaderField: "Authorization")
    ...
    public func callPostWSWithModel<T: Decodable>(_ url_String: String, parameters: [String: Any]?, filePathArr arrFilePath: [[String: Any]]?, model: T.Type, completion: @escaping (MultiPartResult<T, GeneralResponseModel>) -> Void) {
    ...
    fileprivate func createBody(withBoundary boundary: String, parameters: [String: Any]?, paths: [[String: Any]]?) -> Data {
        ...
        if let paths = paths {
            for pathDic in paths {
                for (index, data) in (pathDic[MultiPart.data] as! [Data]).enumerated() {
                    
                    let mimetype: String = (pathDic[MultiPart.mime] as? [String])?[index] ?? ""
                    let aFileName = UUID().uuidString + "." + ((pathDic[MultiPart.fileextension] as? [String])?[index] ?? "")
                        httpBody.append("--\(boundary)\r\n".data(using: String.Encoding.utf8)!)
                        httpBody.append("Content-Disposition: form-data; name=\"\(pathDic[MultiPart.fieldName] ?? "")\"; filename=\"\(aFileName)\"\r\n".data(using: String.Encoding.utf8)!)
...
fileprivate func mainThread(_ completion: @escaping () -> Void) {
